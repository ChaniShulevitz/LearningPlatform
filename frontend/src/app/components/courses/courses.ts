...
export class CoursesComponent implements OnInit {
  ...
  createLesson() {
    ...
    if (!this.userPrompt || !this.userPrompt.trim() || !this.selectedCategoryId) {
      alert('אנא ודאי שבחרת קטגוריה והקלדת שאלה ברורה עבור ה-AI.');
      return;
    }
    ...
    this.http.post<any>(this.apiUrl, requestBody).subscribe({
      next: (response) => {
        ...
        this.aiExplanation = response.aiResponse || response.explanation || response.text || response;
      ...
      error: (err) => {
        ...
        alert('אופס! שגיאה בחיבור למנוע ה-AI. ודאי שהשרת פועל ברקע ושהגדרת מפתח OpenAI ב-env.');
      }
    });
  }
}